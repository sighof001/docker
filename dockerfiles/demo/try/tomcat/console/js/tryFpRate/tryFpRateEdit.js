...
function returnPage(){
	var r = cxt + '/tryFpRate/tryFpRateList.do';
	if(returnType == '1'){
...
function initValidator(){
	var formUrl = '';
	if(op == const_op_add){
		formUrl = cxt + "/tryFpRate/addTryFpRate.do";
	}else if(op == const_op_update){
		formUrl = cxt + "/tryFpRate/updateTryFpRate.do";
	}
	 ...
	 * 校验利率
	 */
	$("#rate").formValidator({onFocus:"请输入利率",onCorrect:"&nbsp;",tipCss:tipCss,onShow:"&nbsp;"})
	.inputValidator({min:1,onError:"利率不能为空"}).inputValidator({
		max:22,
		onError:"利率不能超过22个字符"
	})
	.regexValidator({          
		regExp:regexEnum.newRate,             
		onError:"利率是数字,最多保留到小数点后9位小数"
	});
	/**
	 * 校验本金
	 */
	$("#principal").formValidator({onFocus:"请输入本金",onCorrect:"&nbsp;",tipCss:tipCss,onShow:"&nbsp;"})
	.inputValidator({min:1,onError:"本金不能为空"}).inputValidator({
		max:22,
		onError:"本金不能超过22个字符"
	})
	.regexValidator({          
		regExp:regexEnum.newRate1,             
		onError:"本金是数字,最多保留到小数点后4位小数"      
	});
	/**
	 * 校验天数
	 */
	$("#days").formValidator({onFocus:"请输入天数",onCorrect:"&nbsp;",tipCss:tipCss,onShow:"&nbsp;"})
	.inputValidator({min:1,onError:"天数不能为空"}).inputValidator({
		max:22,
		onError:"天数不能超过22个字符"
	})
	.regexValidator({          
