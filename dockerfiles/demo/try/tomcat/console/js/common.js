...
function requestData(url,reqData,toUrl){
	util.getServerData(url, reqData,function(data) {
		if(data.success){
			$("#success-hint").removeClass("hidden");
			$("#error-hint").addClass("hidden");
			$("#success-hint").html('成功');
			if(toUrl){
...
/**
 * 把对象转化为url请求参数 如｛aaa:bbb,ccc:ddd｝ --> aaa=bbb&ccc=ddd
 * @param o
 * @returns url参数，如果o不是对象,返回空字符串
 */
function objectToParam(o){
	if(typeof(o) != 'object'){
		return "";
	}
