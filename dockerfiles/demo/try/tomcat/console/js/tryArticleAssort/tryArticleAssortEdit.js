...
function returnPage(){
	var r = cxt + '/tryArticleAssort/tryArticleAssortList.do';
	if(returnType == '1'){
...
function initValidator(){
	var formUrl = '';
	if(op == const_op_add){
		formUrl = cxt + "/tryArticleAssort/addTryArticleAssort.do";
	}else if(op == const_op_update){
		formUrl = cxt + "/tryArticleAssort/updateTryArticleAssort.do";
	}
	 ...
	 * 校验栏目名称
	 */
	$("#themeName").formValidator({onFocus:"请输入栏目名称",onCorrect:"&nbsp;",tipCss:tipCss,onShow:"&nbsp;"})
	.inputValidator({min:1,onError:"栏目名称不能为空"}).inputValidator({
		max:50,
		onError:"栏目名称不能超过50个字符"
	});
	/**
	 * 校验栏目key
	 */
	$("#themeKey").formValidator({onFocus:"请输入栏目key",onCorrect:"&nbsp;",tipCss:tipCss,onShow:"&nbsp;"})
	.inputValidator({min:1,onError:"栏目key不能为空"}).inputValidator({
		max:22,
		onError:"栏目key不能超过22个字符"
	})
	.regexValidator({          
		regExp:regexEnum.letter,          
		onError:"栏目key,必须是英文字母"      
	});
	/**
	 * 校验排序
	 */
	$("#sequence").formValidator({onFocus:"请输入排序",onCorrect:"&nbsp;",tipCss:tipCss,onShow:"&nbsp;"})
	.inputValidator({min:1,onError:"排序不能为空"}).inputValidator({
		max:5,
		onError:"排序不能超过5个字符"
	})
	.regexValidator({          
