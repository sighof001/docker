...
//					this.fillData(this.options.data);	
		...
		,initEvent: function() {
			 ...
			 * 改变pageSize事件
			 */
			this.getPagePanel().find("#page-size").on("click", function() {
				this.cur_page = 1;
				$this.fillDataFromServer();
			});
			...
			function sortEvent(el) {
				if (el.closest("th").length == 1) {
					...
					i.closest("tr").find("i").each(function() {
						...
						$(this).attr("class", "icon-sort");
					});
					
					if (i.hasClass("icon-sort")) {
						// 升序
						i.removeClass("icon-sort");
						i.addClass("icon-sort-up");
						$this.orderby = el.attr("name") + " asc";
						$this.fillDataFromServer();
					}else if (i.hasClass("icon-sort-up")) {
						// 降序
						i.removeClass("icon-sort-up");
						i.addClass("icon-sort-down");
						$this.orderby = el.attr("name") + " desc";
						$this.fillDataFromServer();
					}else if (i.hasClass("icon-sort-down")) {
						// 无序
						i.removeClass("icon-sort-down");
			...
			function operateEvent(el) {
				...
				if ((td.length == 1) && (td.attr("name") == "table-op")) {
					...
					if (o != null && o.handler != null && "function" == typeof o.handler) {
						o.handler.call($this, $this.getRowData(btn));
					};
				};
			};
		}
	...
	$.fn.frame.ui.table.defaults = {
		...
		tableClass: '', 
		tr_bg2: '#FFFFF0', 		// 间隔行背景色（当浏览器不支持HTML5时生效） 
		url: '',				// 获取每页数据
		rowsurl: '',			// 获取总页数和总行数
		pageName: 'page',
		pageSizeName: 'limit',
