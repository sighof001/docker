...
//		document.getElementById()
		$("#nav_menu ul").on("click", "li",function(e,callback){
			...
			if("undefined" == typeof demo_version){
				header.initSideMenu($this,callback);
	...
	createTopMenu: function(data) {
		...
		for (var i=0;i<len;i++){
			...
			html.push("<li><a style='color: #000' href='#' id='"+id+"' title='"+name+"' url='"+data[i].url+"' >");
			html.push(name);
			html.push("</a></li>");
		}
		...
		/*登录进去默认打开第一个菜单
		$("#nav_menu ul li:first").trigger('click',function(){
			var first = $('.main-left-menu li:first a');
			header.sideMenuClick({
				id:first.get(0).id,
				name:first.find('span').text(),
				pid:'',
				url:first.attr('url')
			},false,false);
		});
	...
	initSideMenu: function(obj,callback) {
