...
<%-- 				设置用户组所属用户 <small>设置用户组 [<%=name%>] 关联的用户</small> --%>
					设置用户组所属用户 <small>设置用户组关联的用户</small>
			</h3>
...
<script>
	...
	function pullLeft(node) {
		...
		if (node && n.length ==0)
			n.push($(node).data());
		if (n.length > 0) {
			util.getServerData("user/remove_group_users.do", {groupId: '<%=id%>', userIds: buildIds(n)}, function(data) {
				if (data.rs_code == "0") {
	...
	function pullRight(node) {
		...
		if (n.length > 0) {
			util.getServerData("user/add_group_users.do", {groupId: '<%=id%>', userIds: buildIds(n)}, function(data) {
				if (data.rs_code == "0") {
