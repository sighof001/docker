...
$(document).ready(function(){
	...
	$("#table").frametable({
		columns: [
		          ...
		          {name:'baseTable', text:'表名', width: 200},
		          {name:'createDate', text:'创建时间', width: 200},
...
// 		refresh_at_once:false
	...
	$("#add-btn").on("click", function() {
		util.navigate("/admin/tmpuser/addtmp.jsp?op=add");
	});
	
});

function doEdit(data) {
	util.navigate("/admin/tmpuser/edit.jsp?templateName="+encodeURI(data.name)+"&id="+data.id+"&op=update");
// 	util.getServerData("/tmpuser/getbyid.do?tmpId="+data.id);
}
...
function doPurview(data) {
	var url = "/admin/tmpuser/purview.jsp?templateId="+data.id+"&templateName="+encodeURI(data.name);
	util.navigate(url);
}

function doGroup(data) {
	util.navigate("/admin/orgmgr/group.jsp?id="+data.id+"&groupid="+data.groupId+"&name="+encodeURI(data.caption));
}
