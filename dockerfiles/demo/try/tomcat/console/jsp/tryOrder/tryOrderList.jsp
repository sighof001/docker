...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/tryOrder/tryOrderList.js"></script>
...
<body>
	<div class="panel panel-default no-border">
			...
			<ul class="query-form-wrap form-wrap">
					...
					<li><span class="form_span">类型：</span>
					    <select id='queryTradeType' name='queryTradeType'>
					        ...
					        <c:forEach items="${tradeTypes}" var="item">
					            <option value="${item.dictValue}">${item.text}</option>
					        </c:forEach>
					    </select>
					</li>
					<li><span class="form_span">订单状态：</span>
					    <select id='queryCheckStatusArr' name='queryCheckStatusArr'>
					        <option value="">请选择</option>
					        <c:forEach items="${checkStatusList}" var="item">
					            <option value="${item.dictValue}">${item.text}</option>
					        </c:forEach>
					    </select>
					</li>
       ...
       <!--  <div class="content_title">
            <div class="titleName">资金明细列表</div>
