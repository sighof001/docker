$(document).ready(function() {
	...
	$("#companySubmit").on("click",function(){
		if(checkCompanyName()&&checkNameCompany()&&checkTelCompany()&&checkIndustry()&&checkRegisterAddress()&&checkOfficeAddress()&&checkYears()&&checkAmountCompany()&&checkLoanUseCompany()&&checkLoanTermCompany()&&checkRepaymentSourceCompany()&&checkCheckCodeCompany()){
...
function checkTelCompany(){
	...
	var pattern = /^\d+$/;
	if(!mobile){
		$("#telCompanyText").text("手机号不能为空!");
		return false;
	}
	if(mobile.length!=11){
		$("#telCompanyText").text("手机号码为11位");
		return false;
	}
	if(!pattern.test(mobile)){
		$("#telCompanyText").text("请输入正确的手机号!");
		return false;
	}
	$("#telCompanyText").text("");
	return true;
