 ...
 var clientWidth =document.body.clientWidth;
	...
	var PixelDensity=clientWidth/screenWidth;
     return PixelDensity;
...
function (root, factory) {
	...
	var MobileSelectArea = function () {
		...
		var height_new;
		var add_new;
		if(getPixelDensity()<2)
		{ 
		  mtop_new=30;
		 
         }
		 else if(getPixelDensity()>=2&&getPixelDensity()<3)
		{ 
		  mtop_new=60;
		  
         } 
		 else if(getPixelDensity()>=3)
		{ 
		  mtop_new=90;
	...
	MobileSelectArea.prototype = {
		...
		{ 
		  ...
		  add_new= 180;
				...
				if (top < -$(dl).height() + add_new) {
					top = -$(dl).height() + add_new;
				}
